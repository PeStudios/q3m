...
line 981
;980:
;981:	client = level.clients + clientNum;
ADDRLP4 0
CNSTI4 776
ADDRFP4 0
INDIRI4
...
line 991
;990:
;991:	client->pers.connected = CON_CONNECTED;
ADDRLP4 0
INDIRP4
CNSTI4 468
ADDP4
...
ASGNI4
line 992
;992:	client->pers.enterTime = level.time;
ADDRLP4 0
INDIRP4
CNSTI4 552
ADDP4
...
ASGNI4
line 993
;993:	client->pers.teamState.state = TEAM_BEGIN;
ADDRLP4 0
INDIRP4
CNSTI4 556
ADDP4
...
line 1000
;994:
;995:	// save eflags around this, because changing teams will
;996:	// cause this to happen with a valid entity, and we
;997:	// want to make sure the teleport bit is set right
;998:	// so the viewpoint doesn't interpolate through the
;999:	// world to the new position
;1000:	flags = client->ps.eFlags;
ADDRLP4 8
ADDRLP4 0
INDIRP4
CNSTI4 104
...
ASGNI4
line 1001
;1001:	memset( &client->ps, 0, sizeof( client->ps ) );
ADDRLP4 0
INDIRP4
ARGP4
CNSTI4 0
...
line 1005
;1003:
;1004:	// locate ent at a spawn point
;1005:	ClientSpawn( ent );
ADDRLP4 4
INDIRP4
ARGP4
ADDRGP4 ClientSpawn
...
line 1007
;1006:
;1007:	if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
ADDRLP4 0
INDIRP4
CNSTI4 616
ADDP4
...
line 1009
;1008:		// send event
;1009:		tent = G_TempEntity( ent->client->ps.origin, EV_PLAYER_TELEPORT_IN );
ADDRLP4 4
INDIRP4
CNSTI4 516
ADDP4
...
ASGNP4
line 1010
;1010:		tent->s.clientNum = ent->s.clientNum;
ADDRLP4 20
CNSTI4 168
ASGNI4
ADDRLP4 12
...
line 1012
;1011:
;1012:		if ( g_gametype.integer != GT_TOURNAMENT  ) {
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 1
EQI4 $348
line 1013
;1013:			trap_SendServerCommand( -1, va("print \"%s" S_COLOR_WHITE " entered the game\n\"", client->pers.netname) );
ADDRGP4 $351
ARGP4
ADDRLP4 0
INDIRP4
...
LABELV $346
line 1016
;1016:	G_LogPrintf( "ClientBegin: %i\n", clientNum );
ADDRGP4 $352
ARGP4
ADDRFP4 0
INDIRI4
...
line 1019
;1017:
;1018:	// count current clients and rank for scoreboard
;1019:	CalculateRanks();
ADDRGP4 CalculateRanks
CALLV
pop
...
;1024:ClientSpawn
;1025:
;1026:Called every time a client is placed fresh in the world:
;1027:after the first ClientBegin, and after each respawn
;1028:Initializes all non-persistant parts of playerState
;1029:============
;1030:*/
;1031:void ClientSpawn(gentity_t *ent) {
line 1047
;1032:	int		index;
;1033:	vec3_t	spawn_origin, spawn_angles;
;1034:	gclient_t	*client;
;1035:	int		i;
;1036:	clientPersistant_t	saved;
;1037:	clientSession_t		savedSess;
;1038:	int		persistant[MAX_PERSISTANT];
;1039:	gentity_t	*spawnPoint;
;1040:	int		flags;
;1041:	int		savedPing;
;1042://	char	*savedAreaBits;
;1043:	int		accuracy_hits, accuracy_shots;
;1044:	int		eventSequence;
;1045:	char	userinfo[MAX_INFO_STRING];
;1046:
;1047:	index = ent - g_entities;
ADDRLP4 100
ADDRFP4 0
INDIRP4
CVPU4 4
...
line 1053
;1049:
;1050:	// find a spawn point
;1051:	// do it before setting health back up, so farthest
;1052:	// ranging doesn't count this client
...
NEI4 $354
line 1054
;1054:		spawnPoint = SelectSpectatorSpawnPoint ( 
ADDRLP4 76
ARGP4
ADDRLP4 88
ARGP4
...
ASGNP4
line 1056
;1055:						spawn_origin, spawn_angles);
;1056:	} else if (g_gametype.integer >= GT_CTF ) {
ADDRGP4 $355
JUMPV
LABELV $354
ADDRGP4 g_gametype+12
...
LTI4 $356
line 1058
;1057:		// all base oriented team games use the CTF spawn points
;1058:		spawnPoint = SelectCTFSpawnPoint ( 
ADDRLP4 4
INDIRP4
CNSTI4 616
ADDP4
...
ASGNP4
line 1062
;1059:						client->sess.sessionTeam, 
;1060:						client->pers.teamState.state, 
;1061:						spawn_origin, spawn_angles);
;1062:	} else {
ADDRGP4 $357
JUMPV
LABELV $356
...
;1063:		do {
line 1065
;1064:			// the first spawn should be at a good looking spot
;1065:			if ( !client->pers.initialSpawn && client->pers.localClient ) {
ADDRLP4 1328
CNSTI4 0
ASGNI4
ADDRLP4 4
...
EQI4 $362
line 1066
;1066:				client->pers.initialSpawn = qtrue;
ADDRLP4 4
INDIRP4
CNSTI4 500
ADDP4
...
ASGNI4
line 1067
;1067:				spawnPoint = SelectInitialSpawnPoint( spawn_origin, spawn_angles );
ADDRLP4 76
ARGP4
ADDRLP4 88
ARGP4
...
LABELV $362
line 1070
;1069:				// don't spawn near existing origin if possible
;1070:				spawnPoint = SelectSpawnPoint ( 
ADDRLP4 4
INDIRP4
CNSTI4 20
ADDP4
...
ASGNP4
line 1073
;1071:					client->ps.origin, 
;1072:					spawn_origin, spawn_angles);
;1073:			}
LABELV $363
line 1077
;1074:
;1075:			// Tim needs to prevent bots from spawning at the initial point
;1076:			// on q3dm0...
;1077:			if ( ( spawnPoint->flags & FL_NO_BOTS ) && ( ent->r.svFlags & SVF_BOT ) ) {
ADDRLP4 1332
CNSTI4 0
ASGNI4
ADDRLP4 72
...
EQI4 $364
line 1078
;1078:				continue;	// try again
ADDRGP4 $360
JUMPV
LABELV $364
line 1081
;1079:			}
;1080:			// just to be symetric, we have a nohumans option...
;1081:			if ( ( spawnPoint->flags & FL_NO_HUMANS ) && !( ent->r.svFlags & SVF_BOT ) ) {
ADDRLP4 1336
CNSTI4 0
ASGNI4
ADDRLP4 72
...
LABELV $355
line 1089
;1089:	client->pers.teamState.state = TEAM_ACTIVE;
ADDRLP4 4
INDIRP4
CNSTI4 556
ADDP4
...
line 1092
;1090:
;1091:	// always clear the kamikaze flag
;1092:	ent->s.eFlags &= ~EF_KAMIKAZE;
ADDRLP4 1324
ADDRFP4 0
INDIRP4
CNSTI4 8
...
line 1096
;1093:
;1094:	// toggle the teleport bit so the client knows to not lerp
;1095:	// and never clear the voted flag
;1096:	flags = ent->client->ps.eFlags & (EF_TELEPORT_BIT | EF_VOTED | EF_TEAMVOTED);
ADDRLP4 104
ADDRFP4 0
INDIRP4
CNSTI4 516
...
line 1101
;1098:
;1099:	// clear everything but the persistant data
;1100:
...
ASGNI4
line 1105
;1104://	savedAreaBits = client->areabits;
;1105:	accuracy_hits = client->accuracy_hits;
ADDRLP4 1312
ADDRLP4 4
INDIRP4
CNSTI4 716
...
ASGNI4
line 1106
;1106:	accuracy_shots = client->accuracy_shots;
ADDRLP4 1316
ADDRLP4 4
INDIRP4
CNSTI4 712
...
LABELV $368
line 1108
;1108:		persistant[i] = client->ps.persistant[i];
ADDRLP4 1328
ADDRLP4 0
INDIRI4
CNSTI4 2
...
LTI4 $368
line 1110
;1110:	eventSequence = client->ps.eventSequence;
ADDRLP4 1320
ADDRLP4 4
INDIRP4
CNSTI4 108
...
line 1112
;1111:
;1112:	memset (client, 0, sizeof(*client)); // bk FIXME: Com_Memset?
ADDRLP4 4
INDIRP4
ARGP4
CNSTI4 0
...
ASGNI4
line 1118
;1117://	client->areabits = savedAreaBits;
;1118:	client->accuracy_hits = accuracy_hits;
ADDRLP4 4
INDIRP4
CNSTI4 716
ADDP4
...
ASGNI4
line 1119
;1119:	client->accuracy_shots = accuracy_shots;
ADDRLP4 4
INDIRP4
CNSTI4 712
ADDP4
...
ASGNI4
line 1120
;1120:	client->lastkilled_client = -1;
ADDRLP4 4
INDIRP4
CNSTI4 720
ADDP4
...
line 1122
;1121:
;1122:	for ( i = 0 ; i < MAX_PERSISTANT ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $372
line 1123
;1123:		client->ps.persistant[i] = persistant[i];
ADDRLP4 1328
ADDRLP4 0
INDIRI4
CNSTI4 2
...
LTI4 $372
line 1125
;1125:	client->ps.eventSequence = eventSequence;
ADDRLP4 4
INDIRP4
CNSTI4 108
ADDP4
...
ASGNI4
line 1127
;1126:	// increment the spawncount so the client will detect the respawn
;1127:	client->ps.persistant[PERS_SPAWN_COUNT]++;
ADDRLP4 1328
ADDRLP4 4
INDIRP4
CNSTI4 264
...
ASGNI4
line 1128
;1128:	client->ps.persistant[PERS_TEAM] = client->sess.sessionTeam;
ADDRLP4 4
INDIRP4
CNSTI4 260
ADDP4
...
line 1130
;1129:
;1130:	client->airOutTime = level.time + 12000;
ADDRLP4 4
INDIRP4
CNSTI4 748
ADDP4
...
line 1132
;1131:
;1132:	trap_GetUserinfo( index, userinfo, sizeof(userinfo) );
ADDRLP4 100
INDIRI4
ARGI4
ADDRLP4 108
...
line 1134
;1133:	// set max health
;1134:	client->pers.maxHealth = atoi( Info_ValueForKey( userinfo, "handicap" ) );
ADDRLP4 108
ARGP4
ADDRGP4 $267
ARGP4
...
ASGNI4
line 1135
;1135:	if ( client->pers.maxHealth < 1 || client->pers.maxHealth > 100 ) {
ADDRLP4 1344
ADDRLP4 4
INDIRP4
CNSTI4 548
...
LABELV $379
line 1136
;1136:		client->pers.maxHealth = 100;
ADDRLP4 4
INDIRP4
CNSTI4 548
ADDP4
...
LABELV $377
line 1139
;1138:	// clear entity values
;1139:	client->ps.stats[STAT_MAX_HEALTH] = client->pers.maxHealth;
ADDRLP4 4
INDIRP4
CNSTI4 208
ADDP4
...
line 1142
;1141:
;1142:	ent->s.groundEntityNum = ENTITYNUM_NONE;
ADDRFP4 0
INDIRP4
CNSTI4 148
ADDP4
...
ASGNI4
line 1143
;1143:	ent->client = &level.clients[index];
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
ASGNP4
line 1147
;1147:	ent->r.contents = CONTENTS_BODY;
ADDRFP4 0
INDIRP4
CNSTI4 460
ADDP4
...
ASGNI4
line 1148
;1148:	ent->clipmask = MASK_PLAYERSOLID;
ADDRFP4 0
INDIRP4
CNSTI4 572
ADDP4
...
line 1154
;1153:	
;1154:	VectorCopy (playerMins, ent->r.mins);
ADDRFP4 0
INDIRP4
CNSTI4 436
ADDP4
...
ASGNB 12
line 1155
;1155:	VectorCopy (playerMaxs, ent->r.maxs);
ADDRFP4 0
INDIRP4
CNSTI4 448
ADDP4
...
line 1157
;1156:
;1157:	client->ps.clientNum = index;
ADDRLP4 4
INDIRP4
CNSTI4 140
ADDP4
...
line 1159
;1158:
;1159:	client->ps.stats[STAT_WEAPONS] = ( 1 << WP_MACHINEGUN );
ADDRLP4 4
INDIRP4
CNSTI4 192
ADDP4
...
NEI4 $381
line 1161
;1161:		client->ps.ammo[WP_MACHINEGUN] = 50;
ADDRLP4 4
INDIRP4
CNSTI4 384
ADDP4
...
LABELV $381
line 1163
;1163:		client->ps.ammo[WP_MACHINEGUN] = 100;
ADDRLP4 4
INDIRP4
CNSTI4 384
ADDP4
...
line 1166
;1165:
;1166:	client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_GAUNTLET );
ADDRLP4 1352
ADDRLP4 4
INDIRP4
CNSTI4 192
...
ASGNI4
line 1167
;1167:	client->ps.ammo[WP_GAUNTLET] = -1;
ADDRLP4 4
INDIRP4
CNSTI4 380
ADDP4
...
ASGNI4
line 1168
;1168:	client->ps.ammo[WP_GRAPPLING_HOOK] = -1;
ADDRLP4 4
INDIRP4
CNSTI4 416
ADDP4
...
line 1171
;1169:
;1170:	// health will count down towards max_health
;1171:	ent->health = client->ps.stats[STAT_HEALTH] = client->ps.stats[STAT_MAX_HEALTH] + 25;
ADDRLP4 1360
ADDRLP4 4
INDIRP4
CNSTI4 208
...
line 1173
;1172:
;1173:	G_SetOrigin( ent, spawn_origin );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 76
...
pop
line 1174
;1174:	VectorCopy( spawn_origin, client->ps.origin );
ADDRLP4 4
INDIRP4
CNSTI4 20
ADDP4
...
line 1177
;1175:
;1176:	// the respawned flag will be cleared after the attack and jump keys come up
;1177:	client->ps.pm_flags |= PMF_RESPAWNED;
ADDRLP4 1364
ADDRLP4 4
INDIRP4
CNSTI4 12
...
line 1179
;1178:
;1179:	trap_GetUsercmd( client - level.clients, &ent->client->pers.cmd );
ADDRLP4 4
INDIRP4
CVPU4 4
ADDRGP4 level
...
pop
line 1180
;1180:	SetClientViewAngle( ent, spawn_angles );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 88
...
line 1182
;1181:
;1182:	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 1189
;1187:
;1188:		// force the base weapon up
;1189:		client->ps.weapon = WP_MACHINEGUN;
ADDRLP4 4
INDIRP4
CNSTI4 144
ADDP4
...
ASGNI4
line 1190
;1190:		client->ps.weaponstate = WEAPON_READY;
ADDRLP4 4
INDIRP4
CNSTI4 148
ADDP4
...
line 1195
;1193:
;1194:	// don't allow full run speed for a bit
;1195:	client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
ADDRLP4 1368
ADDRLP4 4
INDIRP4
CNSTI4 12
...
line 1198
;1197:
;1198:	client->respawnTime = level.time;
ADDRLP4 4
INDIRP4
CNSTI4 732
ADDP4
...
ASGNI4
line 1199
;1199:	client->inactivityTime = level.time + g_inactivity.integer * 1000;
ADDRLP4 4
INDIRP4
CNSTI4 736
ADDP4
...
line 1203
;1201:
;1202:	// set default animations
;1203:	client->ps.torsoAnim = TORSO_STAND;
ADDRLP4 4
INDIRP4
CNSTI4 84
ADDP4
...
ASGNI4
line 1204
;1204:	client->ps.legsAnim = LEGS_IDLE;
ADDRLP4 4
INDIRP4
CNSTI4 76
ADDP4
...
line 1206
;1205:
;1206:	if ( level.intermissiontime ) {
ADDRGP4 level+9128
INDIRI4
CNSTI4 0
EQI4 $389
line 1207
;1207:		MoveClientToIntermission( ent );
ADDRFP4 0
INDIRP4
ARGP4
...
LABELV $389
line 1210
;1209:		// fire the targets of the spawn point
;1210:		G_UseTargets( spawnPoint, ent );
ADDRLP4 72
INDIRP4
ARGP4
ADDRFP4 0
...
line 1214
;1211:
;1212:		// select the highest weapon number available, after any
;1213:		// spawn given items have fired
;1214:		client->ps.weapon = 1;
ADDRLP4 4
INDIRP4
CNSTI4 144
ADDP4
...
ASGNI4
line 1215
;1215:		for ( i = WP_NUM_WEAPONS - 1 ; i > 0 ; i-- ) {
ADDRLP4 0
CNSTI4 10
ASGNI4
LABELV $392
line 1216
;1216:			if ( client->ps.stats[STAT_WEAPONS] & ( 1 << i ) ) {
ADDRLP4 4
INDIRP4
CNSTI4 192
ADDP4
...
line 1225
;1222:
;1223:	// run a client frame to drop exactly to the floor,
;1224:	// initialize animations and other things
;1225:	client->ps.commandTime = level.time - 100;
ADDRLP4 4
INDIRP4
ADDRGP4 level+32
INDIRI4
...
ASGNI4
line 1226
;1226:	ent->client->pers.cmd.serverTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
ASGNI4
line 1227
;1227:	ClientThink( ent-g_entities );
ADDRFP4 0
INDIRP4
CVPU4 4
ADDRGP4 g_entities
...
line 1230
;1228:
;1229:	// positively link the client, even if the command times are weird
...
EQI4 $400
line 1231
;1231:		BG_PlayerStateToEntityState( &client->ps, &ent->s, qtrue );
ADDRLP4 4
INDIRP4
ARGP4
ADDRFP4 0
...
line 1237
;1235:
;1236:	// run the presend to set anything else
;1237:	ClientEndFrame( ent );
ADDRFP4 0
INDIRP4
ARGP4
...
line 1240
;1238:
;1239:	// clear entity state values
;1240:	BG_PlayerStateToEntityState( &client->ps, &ent->s, qtrue );
ADDRLP4 4
INDIRP4
ARGP4
ADDRFP4 0
...
;1247:
;1248:Called when a player drops from the server.
;1249:Will not be called between levels.
;1250:
;1251:This should NOT be called directly by any game logic,
;1252:call trap_DropClient(), which will call this and do
;1253:server system housekeeping.
;1254:============
;1255:*/
;1256:void ClientDisconnect( int clientNum ) {
line 1263
;1257:	gentity_t	*ent;
;1258:	gentity_t	*tent;
;1259:	int			i;
;1260:
;1261:	// cleanup if we are kicking a bot that
;1262:	// hasn't spawned yet
;1263:	G_RemoveQueuedBotBegin( clientNum );
ADDRFP4 0
INDIRI4
ARGI4
...
;1268:	}
;1269:
;1270:	// stop any following clients
;1271:	for ( i = 0 ; i < level.maxclients ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $408
...
LABELV $405
line 1272
;1272:		if ( level.clients[i].sess.sessionTeam == TEAM_SPECTATOR
ADDRLP4 12
CNSTI4 776
ADDRLP4 0
INDIRI4
...
NEI4 $410
line 1274
;1273:			&& level.clients[i].sess.spectatorState == SPECTATOR_FOLLOW
;1274:			&& level.clients[i].sess.spectatorClient == clientNum ) {
line 1275
;1275:			StopFollowing( &g_entities[i] );
CNSTI4 808
ADDRLP4 0
INDIRI4
MULI4
...
line 1280
;1278:
;1279:	// send effect if they were completely connected
;1280:	if ( ent->client->pers.connected == CON_CONNECTED 
ADDRLP4 12
ADDRLP4 4
INDIRP4
CNSTI4 516
...
EQI4 $412
line 1281
;1281:		&& ent->client->sess.sessionTeam != TEAM_SPECTATOR ) {
line 1282
;1282:		tent = G_TempEntity( ent->client->ps.origin, EV_PLAYER_TELEPORT_OUT );
ADDRLP4 4
INDIRP4
CNSTI4 516
ADDP4
...
ASGNP4
line 1283
;1283:		tent->s.clientNum = ent->s.clientNum;
ADDRLP4 20
CNSTI4 168
ASGNI4
ADDRLP4 8
...
line 1287
;1284:
;1285:		// They don't get to take powerups with them!
;1286:		// Especially important for stuff like CTF flags
;1287:		TossClientItems( ent );
ADDRLP4 4
INDIRP4
ARGP4
...
;1289:		TossClientPersistantPowerups( ent );
;1290:		if( g_gametype.integer == GT_HARVESTER ) {
;1291:			TossClientCubes( ent );
;1292:		}
;1293:#endif
;1294:
;1295:	}
...
line 1297
;1296:
;1297:	G_LogPrintf( "ClientDisconnect: %i\n", clientNum );
ADDRGP4 $414
ARGP4
ADDRFP4 0
INDIRI4
...
line 1300
;1298:
;1299:	// if we are playing in tourney mode and losing, give a win to the other player
;1300:	if ( (g_gametype.integer == GT_TOURNAMENT )
ADDRGP4 g_gametype+12
INDIRI4
CNSTI4 1
NEI4 $415
...
NEI4 $415
line 1302
;1301:		&& !level.intermissiontime
;1302:		&& !level.warmupTime && level.sortedClients[1] == clientNum ) {
line 1303
;1303:		level.clients[ level.sortedClients[0] ].sess.wins++;
ADDRLP4 20
CNSTI4 776
ADDRGP4 level+84
INDIRI4
...
ASGNI4
line 1304
;1304:		ClientUserinfoChanged( level.sortedClients[0] );
ADDRGP4 level+84
INDIRI4
ARGI4
ADDRGP4 ClientUserinfoChanged
...
ASGNI4
line 1310
;1310:	ent->classname = "disconnected";
ADDRLP4 4
INDIRP4
CNSTI4 524
ADDP4
...
ASGNP4
line 1311
;1311:	ent->client->pers.connected = CON_DISCONNECTED;
ADDRLP4 4
INDIRP4
CNSTI4 516
ADDP4
...
ASGNI4
line 1312
;1312:	ent->client->ps.persistant[PERS_TEAM] = TEAM_FREE;
ADDRLP4 4
INDIRP4
CNSTI4 516
ADDP4
...
ASGNI4
line 1313
;1313:	ent->client->sess.sessionTeam = TEAM_FREE;
ADDRLP4 4
INDIRP4
CNSTI4 516
ADDP4
...
line 1315
;1314:
;1315:	trap_SetConfigstring( CS_PLAYERS + clientNum, "");
ADDRFP4 0
INDIRI4
CNSTI4 544
ADDI4
...
EQI4 $426
line 1320
;1320:		BotAIShutdownClient( clientNum, qfalse );
ADDRFP4 0
INDIRI4
ARGI4
CNSTI4 0
