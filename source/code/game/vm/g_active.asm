...
ASGNI4
line 899
;899:	pm.noFootsteps = ( g_dmflags.integer & DF_NO_FOOTSTEPS ) > 0;
ADDRGP4 g_dmflags+12
INDIRI4
CNSTI4 32
BANDI4
...
line 901
;900:
;901:	pm.pmove_fixed = pmove_fixed.integer | client->pers.pmoveFixed;
ADDRLP4 4+216
ADDRGP4 pmove_fixed+12
INDIRI4
ADDRLP4 0
...
ASGNI4
line 902
;902:	pm.pmove_msec = pmove_msec.integer;
ADDRLP4 4+220
ADDRGP4 pmove_msec+12
INDIRI4
ASGNI4
line 904
;903:
;904:	VectorCopy( client->ps.origin, client->oldOrigin );
ADDRLP4 0
INDIRP4
CNSTI4 668
ADDP4
...
line 921
;905:
;906:#ifdef MISSIONPACK
;907:		if (level.intermissionQueued != 0 && g_singlePlayer.integer) {
;908:			if ( level.time - level.intermissionQueued >= 1000  ) {
;909:				pm.cmd.buttons = 0;
;910:				pm.cmd.forwardmove = 0;
;911:				pm.cmd.rightmove = 0;
;912:				pm.cmd.upmove = 0;
;913:				if ( level.time - level.intermissionQueued >= 2000 && level.time - level.intermissionQueued <= 2500 ) {
;914:					trap_SendConsoleCommand( EXEC_APPEND, "centerview\n");
;915:				}
;916:				ent->client->ps.pm_type = PM_SPINTERMISSION;
;917:			}
;918:		}
;919:		Pmove (&pm);
;920:#else
...
;922:#endif
;923:
;924:	// save results of pmove
;925:	if ( ent->client->ps.eventSequence != oldEventSequence ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
LABELV $339
line 928
;928:	if (g_smoothClients.integer) {
ADDRGP4 g_smoothClients+12
INDIRI4
CNSTI4 0
EQI4 $342
line 929
;929:		BG_PlayerStateToEntityStateExtraPolate( &ent->client->ps, &ent->s, ent->client->ps.commandTime, qtrue );
ADDRLP4 284
ADDRFP4 0
INDIRP4
ASGNP4
...
;931:	else {
line 932
;932:		BG_PlayerStateToEntityState( &ent->client->ps, &ent->s, qtrue );
ADDRLP4 284
ADDRFP4 0
INDIRP4
ASGNP4
...
LABELV $343
line 934
;934:	SendPendingPredictableEvents( &ent->client->ps );
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 936
;935:
;936:	if ( !( ent->client->ps.eFlags & EF_FIRING ) ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
NEI4 $345
line 937
;937:		client->fireHeld = qfalse;		// for grapple
ADDRLP4 0
INDIRP4
CNSTI4 756
ADDP4
...
line 941
;939:
;940:	// use the snapped origin for linking so it matches client predicted versions
;941:	VectorCopy( ent->s.pos.trBase, ent->r.currentOrigin );
ADDRLP4 284
ADDRFP4 0
INDIRP4
ASGNP4
...
line 943
;942:
;943:	VectorCopy (pm.mins, ent->r.mins);
ADDRFP4 0
INDIRP4
CNSTI4 436
ADDP4
...
ASGNB 12
line 944
;944:	VectorCopy (pm.maxs, ent->r.maxs);
ADDRFP4 0
INDIRP4
CNSTI4 448
ADDP4
...
line 946
;945:
;946:	ent->waterlevel = pm.waterlevel;
ADDRFP4 0
INDIRP4
CNSTI4 788
ADDP4
...
ASGNI4
line 947
;947:	ent->watertype = pm.watertype;
ADDRFP4 0
INDIRP4
CNSTI4 784
ADDP4
...
line 950
;948:
;949:	// execute client events
;950:	ClientEvents( ent, oldEventSequence );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 244
...
line 953
;951:
;952:	// link entity now, after any personal teleporters have been used
...
line 959
;957:
;958:	// NOTE: now copy the exact origin over otherwise clients can be snapped into solid
;959:	VectorCopy( ent->client->ps.origin, ent->r.currentOrigin );
ADDRLP4 288
ADDRFP4 0
INDIRP4
ASGNP4
...
line 962
;960:
;961:	//test for solid areas in the AAS file
;962:	BotTestAAS(ent->r.currentOrigin);
ADDRFP4 0
INDIRP4
CNSTI4 488
ADDP4
...
line 965
;963:
;964:	// touch other objects
;965:	ClientImpacts( ent, &pm );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 4
...
line 968
;966:
;967:	// save results of triggers and client events
;968:	if (ent->client->ps.eventSequence != oldEventSequence) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 973
;971:
;972:	// swap and latch button actions
;973:	client->oldbuttons = client->buttons;
ADDRLP4 0
INDIRP4
CNSTI4 660
ADDP4
...
ASGNI4
line 974
;974:	client->buttons = ucmd->buttons;
ADDRLP4 0
INDIRP4
CNSTI4 656
ADDP4
...
ASGNI4
line 975
;975:	client->latched_buttons |= client->buttons & ~client->oldbuttons;
ADDRLP4 300
ADDRLP4 0
INDIRP4
CNSTI4 664
...
line 978
;976:
;977:	// check for respawning
;978:	if ( client->ps.stats[STAT_HEALTH] <= 0 ) {
ADDRLP4 0
INDIRP4
CNSTI4 184
ADDP4
...
GTI4 $356
line 980
;979:		// wait for the attack button to be pressed
;980:		if ( level.time > client->respawnTime ) {
ADDRGP4 level+32
INDIRI4
ADDRLP4 0
INDIRP4
...
LEI4 $260
line 982
;981:			// forcerespawn is to prevent users from waiting out powerups
;982:			if ( g_forcerespawn.integer > 0 && 
...
LEI4 $361
line 983
;983:				( level.time - client->respawnTime ) > g_forcerespawn.integer * 1000 ) {
line 984
;984:				respawn( ent );
ADDRFP4 0
INDIRP4
...
;986:			}
;987:		
;988:			// pressing attack or use is the normal respawn method
;989:			if ( ucmd->buttons & ( BUTTON_ATTACK | BUTTON_USE_HOLDABLE ) ) {
ADDRLP4 236
INDIRP4
CNSTI4 16
ADDP4
...
;994:	}
;995:
;996:	// perform once-a-second actions
;997:	ClientTimerActions( ent, msec );
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 240
...
;1002:ClientThink
;1003:
;1004:A new command has arrived from the client
;1005:==================
;1006:*/
;1007:void ClientThink( int clientNum ) {
line 1010
;1008:	gentity_t *ent;
;1009:
;1010:	ent = g_entities + clientNum;
ADDRLP4 0
CNSTI4 808
ADDRFP4 0
INDIRI4
...
ASGNP4
line 1011
;1011:	trap_GetUsercmd( clientNum, &ent->client->pers.cmd );
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 0
...
line 1015
;1012:
;1013:	// mark the time we got info, so we can display the
;1014:	// phone jack if they don't get any for a while
;1015:	ent->client->lastCmdTime = level.time;
ADDRLP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 1017
;1016:
;1017:	if ( !(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer ) {
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRLP4 0
...
;1021:
;1022:
;1023:void G_RunClient( gentity_t *ent ) {
line 1024
;1024:	if ( !(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRFP4 0
...
line 1027
;1026:	}
;1027:	ent->client->pers.cmd.serverTime = level.time;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
;1032:/*
;1033:==================
;1034:SpectatorClientEndFrame
;1035:
;1036:==================
;1037:*/
;1038:void SpectatorClientEndFrame( gentity_t *ent ) {
line 1042
;1039:	gclient_t	*cl;
;1040:
;1041:	// if we are doing a chase cam or a remote view, grab the latest info
;1042:	if ( ent->client->sess.spectatorState == SPECTATOR_FOLLOW ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
NEI4 $379
line 1045
;1043:		int		clientNum, flags;
;1044:
;1045:		clientNum = ent->client->sess.spectatorClient;
ADDRLP4 4
ADDRFP4 0
INDIRP4
CNSTI4 516
...
line 1048
;1046:
;1047:		// team follow1 and team follow2 go to whatever clients are playing
;1048:		if ( clientNum == -1 ) {
ADDRLP4 4
INDIRI4
CNSTI4 -1
NEI4 $381
line 1049
;1049:			clientNum = level.follow1;
ADDRLP4 4
ADDRGP4 level+340
INDIRI4
ASGNI4
line 1050
;1050:		} else if ( clientNum == -2 ) {
ADDRGP4 $382
JUMPV
LABELV $381
ADDRLP4 4
...
LTI4 $387
line 1054
;1054:			cl = &level.clients[ clientNum ];
ADDRLP4 0
CNSTI4 776
ADDRLP4 4
INDIRI4
...
ASGNP4
line 1055
;1055:			if ( cl->pers.connected == CON_CONNECTED && cl->sess.sessionTeam != TEAM_SPECTATOR ) {
ADDRLP4 12
ADDRLP4 0
INDIRP4
ASGNP4
...
EQI4 $389
line 1056
;1056:				flags = (cl->ps.eFlags & ~(EF_VOTED | EF_TEAMVOTED)) | (ent->client->ps.eFlags & (EF_VOTED | EF_TEAMVOTED));
ADDRLP4 16
CNSTI4 104
ASGNI4
ADDRLP4 8
...
ASGNB 468
line 1058
;1058:				ent->client->ps.pm_flags |= PMF_FOLLOW;
ADDRLP4 20
ADDRFP4 0
INDIRP4
CNSTI4 516
...
ASGNI4
line 1059
;1059:				ent->client->ps.eFlags = flags;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
;1061:			} else {
line 1063
;1062:				// drop them to free spectators unless they are dedicated camera followers
;1063:				if ( ent->client->sess.spectatorClient >= 0 ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
LTI4 $391
line 1064
;1064:					ent->client->sess.spectatorState = SPECTATOR_FREE;
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
ASGNI4
line 1065
;1065:					ClientBegin( ent->client - level.clients );
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 1071
;1070:
;1071:	if ( ent->client->sess.spectatorState == SPECTATOR_SCOREBOARD ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
NEI4 $393
line 1072
;1072:		ent->client->ps.pm_flags |= PMF_SCOREBOARD;
ADDRLP4 4
ADDRFP4 0
INDIRP4
CNSTI4 516
...
LABELV $393
line 1074
;1074:		ent->client->ps.pm_flags &= ~PMF_SCOREBOARD;
ADDRLP4 4
ADDRFP4 0
INDIRP4
CNSTI4 516
...
line 1091
;1088:	int			i;
;1089:	clientPersistant_t	*pers;
;1090:
;1091:	if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
NEI4 $396
line 1092
;1092:		SpectatorClientEndFrame( ent );
ADDRFP4 0
INDIRP4
ARGP4
...
line 1099
;1097:
;1098:	// turn off any expired powerups
;1099:	for ( i = 0 ; i < MAX_POWERUPS ; i++ ) {
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $398
line 1100
;1100:		if ( ent->client->ps.powerups[ i ] < level.time ) {
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
GEI4 $402
line 1101
;1101:			ent->client->ps.powerups[ i ] = 0;
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
...
;1106:	// set powerup for player animation
;1107:	if( bg_itemlist[ent->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_GUARD ) {
;1108:		ent->client->ps.powerups[PW_GUARD] = level.time;
;1109:	}
;1110:	if( bg_itemlist[ent->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_SCOUT ) {
;1111:		ent->client->ps.powerups[PW_SCOUT] = level.time;
;1112:	}
;1113:	if( bg_itemlist[ent->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_DOUBLER ) {
;1114:		ent->client->ps.powerups[PW_DOUBLER] = level.time;
;1115:	}
;1116:	if( bg_itemlist[ent->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_AMMOREGEN ) {
;1117:		ent->client->ps.powerups[PW_AMMOREGEN] = level.time;
;1118:	}
;1119:	if ( ent->client->invulnerabilityTime > level.time ) {
;1120:		ent->client->ps.powerups[PW_INVULNERABILITY] = level.time;
;1121:	}
;1122:#endif
;1123:
;1124:	// save network bandwidth
;1125:#if 0
;1126:	if ( !g_synchronousClients->integer && ent->client->ps.pm_type == PM_NORMAL ) {
;1127:		// FIXME: this must change eventually for non-sync demo recording
;1128:		VectorClear( ent->client->ps.viewangles );
;1129:	}
;1130:#endif
;1131:
;1132:	//
;1133:	// If the end of unit layout is displayed, don't give
;1134:	// the player any normal movement attributes
;1135:	//
;1136:	if ( level.intermissiontime ) {
ADDRGP4 level+9128
INDIRI4
CNSTI4 0
EQI4 $405
...
;1138:	}
;1139:
;1140:	// burn from lava, etc
;1141:	P_WorldEffects (ent);
ADDRFP4 0
INDIRP4
ARGP4
...
line 1144
;1142:
;1143:	// apply all the damage taken this frame
;1144:	P_DamageFeedback (ent);
ADDRFP4 0
INDIRP4
ARGP4
...
line 1147
;1145:
;1146:	// add the EF_CONNECTION flag if we haven't gotten commands recently
;1147:	if ( level.time - ent->client->lastCmdTime > 1000 ) {
ADDRGP4 level+32
INDIRI4
ADDRFP4 0
INDIRP4
...
LEI4 $408
line 1148
;1148:		ent->s.eFlags |= EF_CONNECTION;
ADDRLP4 8
ADDRFP4 0
INDIRP4
CNSTI4 8
...
LABELV $408
line 1150
;1150:		ent->s.eFlags &= ~EF_CONNECTION;
ADDRLP4 8
ADDRFP4 0
INDIRP4
CNSTI4 8
...
line 1153
;1152:
;1153:	ent->client->ps.stats[STAT_HEALTH] = ent->health;	// FIXME: get rid of ent->health...
ADDRLP4 8
ADDRFP4 0
INDIRP4
ASGNP4
...
line 1158
;1156:
;1157:	// set the latest infor
;1158:	if (g_smoothClients.integer) {
ADDRGP4 g_smoothClients+12
INDIRI4
CNSTI4 0
EQI4 $411
line 1159
;1159:		BG_PlayerStateToEntityStateExtraPolate( &ent->client->ps, &ent->s, ent->client->ps.commandTime, qtrue );
ADDRLP4 12
ADDRFP4 0
INDIRP4
ASGNP4
...
;1161:	else {
line 1162
;1162:		BG_PlayerStateToEntityState( &ent->client->ps, &ent->s, qtrue );
ADDRLP4 12
ADDRFP4 0
INDIRP4
ASGNP4
...
LABELV $412
line 1164
;1164:	SendPendingPredictableEvents( &ent->client->ps );
ADDRFP4 0
INDIRP4
CNSTI4 516
ADDP4
...
line 1169
;1165:
;1166:	// set the bit for the reachability area the client is currently in
;1167://	i = trap_AAS_PointReachabilityAreaIndex( ent->client->ps.origin );
;1168://	ent->client->areabits[i >> 3] |= 1 << (i & 7);
;1169:}
LABELV $395
